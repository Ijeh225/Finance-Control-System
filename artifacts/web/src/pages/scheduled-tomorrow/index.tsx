...
export default function ScheduledTomorrow() {
  ...
  const { data, isLoading } = useGetScheduledTomorrow(params, {
  ...
  const withdraw = useWithdrawBill({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getGetScheduledTomorrowQueryKey(params) });
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Scheduled Tomorrow</h1>
          {!isLoading && data && (
            <p className="text-sm text-muted-foreground">
              {data.bills?.length ?? 0} payment{(data.bills?.length ?? 0) !== 1 ? "s" : ""} · {formatCurrency(data.total ?? 0)} due tomorrow
            </p>
          )}
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-16 text-center">
              ...
              <p className="text-sm text-muted-foreground font-medium">Nothing scheduled for tomorrow.</p>
            </div>
          ) : (
            <div className="divide-y">
              {data.bills.map(bill => {
                ...
                return (
                  <div
                    ...
                    data-testid={`row-tomorrow-${bill.id}`}
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      {canRemove && (
                        <Button
                          ...
                          data-testid={`button-remove-tomorrow-${bill.id}`}
